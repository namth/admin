            ...
            <?php if(!empty($messages)) echo $messages;?>
                <div class="bottom-spacing">
                    ...
                    <div class="float-right" >
                        <a href="?mod=product&act=insert_group" class="button">
                ...
                <div class="module">
                    ...
                    <div class="module-table-body">
                        ...
                        <table id="myTable" class="tablesorter">
                            ...
                            <tbody>
                            <?php
                                $reusult = mysql_query("SELECT * FROM tbl_category WHERE lang_id = $lang and id <> 1 and grouptype = 1") or die("Query {$q} \n<br/> MySQL Error: " . mysqli_error($dbc));
                                while($rows = mysql_fetch_array($reusult)){
                            ?>
                                <tr>
                                    <td class="align-center"><?php echo $rows['id']; ?></td>
                                    <td><a href="?mod=product&act=edit_group&catid=<?php echo $rows['id']?>"><b><?php echo $rows['name']; ?></b></a></td>
                                    <td><?php echo $rows['seolink']; ?></td>
                                    <td>Sản phẩm</td>
                                    <td>
                                    	<input type="checkbox" />
                                        <a href="?mod=product&act=edit_group&catid=<?php echo $rows['id']?>"><img src="images/pencil.gif" width="16" height="16" alt="edit" /></a>
                                        <?php
                                            $cat_id = $rows['id'];
											...
											$rs_pro = mysql_query("select count(*) as count from  tbl_products where lang_id = $lang  and category_id = $cat_id"); 
                                            ...
                                            if((int)$rows_cat['count'] > 0){
                                             ...
                                             <a href="#" onclick="javascript: alert('Vui lòng xóa danh mục trực thuộc danh mục này. trước khi xóa nó.');"><img src="images/bin.gif" width="16" height="16" alt="delete" /></a>   
                                                <?php
                                            }else if($rows_pro['count']>0){
												?>
												<a href="?mod=product&act=delete_group&catid=<?php echo $rows['id']; ?>" onclick="return confirm('Hiện đang có <?php echo $rows_pro['count'];?> sản phẩm thuộc nhóm nay.\nNếu bạn xóa thì các sản phẩm đó cũng bị xóa.\nBẠN CÓ CHẮC CHẮN XÓA');"><img src="images/bin.gif" width="16" height="16" alt="delete" /></a>
												<?php
												}else{
                                                ?>
                                            <a href="?mod=product&act=delete_group&catid=<?php echo $rows['id']; ?>" onclick="return confirm('BẠN CÓ CHẮC CHẮN MUỐN XÓA KHÔNG????')"><img src="images/bin.gif" width="16" height="16" alt="delete" /></a>
                                                <?php
                                            }
                                        ?>
