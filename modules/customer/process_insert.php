...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    foreach($errors as $check)
        echo $check;
    if(empty($errors)){
        $q = "SELECT * FROM tbl_customer WHERE email='{$email}'";
        echo $q;
        $r = mysql_query($q);
        if(mysql_affected_rows($conn)==0){
            ...
            $q1 ="INSERT INTO `default`.`tbl_customer` (`email`, `pass`, `first_name`, `last_name`, `phone`, `add`, `user_level`, `active`, `reg_date`) VALUES ('{$email}', '{$password}', '{$firstname}', '{$lastname}', '{$phone}', '{$address}', {$userlevel}, '{$active}',  NOW())";
            echo $q1;
            $r1 = mysql_query($q1);
            if(mysql_affected_rows($conn) == 1){
            	...
            	if($chk)
                    header("location: index.php?mod=customer&rp=1&ad=ins&e=1&mn=$email");
            	else
         		    header("location: index.php?mod=customer&rp=1&ad=ins&e=2&mn=$email");
                
            }else{
                header("location: index.php?mod=customer&rp=1&ad=ins&e=4&mn=$email");
            }
        }else{
            header("location: index.php?mod=customer&rp=1&ad=ins&e=3&mn=$email");
        }
    } else{
