    ...
    <div class="grid_12">
        ...
        <?php
            if(isset($_GET['c_id'])&& filter_var($_GET['c_id'], FILTER_VALIDATE_INT, array('min_range'=>0))){
                $c_id = $_GET['c_id'];
                $r = mysql_query("SELECT avatar FROM  tbl_customer WHERE id = {$c_id}");
                $img = mysql_fetch_array($r);
                if(!empty($img['avatar'])) unlink("../uploads/".$img['avatar']);
                $q = "DELETE FROM tbl_customer WHERE id = {$c_id} LIMIT 1";
                //Neu xoa thanh cong thi thong bao ra va xoa het cac danh muc con cua no
                if(mysql_query($q)){
                    header("location: index.php?mod=customer&rp=1&ad=del&mn=$c_id");
                }else{
