...
//Xac nhan bien GET ton tai va dung loai du lieu cho phep.
	...
	if(($_SERVER['REQUEST_METHOD'] == 'POST')&& isset($_POST['submit'])){
		...
		if(empty($errors)){
		$q = "UPDATE tbl_category SET name='{$cat_name}', user_id={$_SESSION['uid']}, level={$parents}, seolink='{$seolink}' 
			  WHERE id={$catid} LIMIT 1";
		$r = mysql_query($q) or die("Query {$q} \n<br/> MySQL Error: " . mysqli_error($dbc));
		
			if(mysql_query($q))
            	header("location: index.php?mod=news&act=list_group&rp=1&ad=edit&mn=$catid");
			else
				header('location: index.php?mod=news&act=list_group&rp=2&ad=edit');
        }else{
            header('location: index.php?mod=news&act=list_group&rp=3&ad=edit');
        }
