...
if(($_SERVER['REQUEST_METHOD'] == 'POST')&& isset($_POST['submit'])){
        ...
        if(empty($errors)){
            $q = "INSERT INTO tbl_category (name, lang_id, user_id, level, seolink, grouptype) VALUES ('{$cat_name}', $lang, {$_SESSION['uid']}, {$parents}, '{$seolink}', 2)";
            if(mysql_query($q))
            	header("location: index.php?mod=news&act=list_group&rp=1&ad=ins&mn=$cat_name");
			else
				header('location: index.php?mod=news&act=list_group&rp=2&ad=ins');
        }else{
            header('location: index.php?mod=news&act=list_group&rp=3&ad=ins');
        }
    }elseif(isset($_POST['cancel'])){
        header('location: index.php?mod=news&act=list_group');
    }
?>
